...
<body>
    ...
    <main>
        ...
        <div class="form-container">
            <form method="POST" enctype="multipart/form-data" action="edit_etape.php?id=<?= $id_etape ?>">
                ...
                <div class="form-group-horizontal">
                    ...
                    <?php
                    // Récupérer tous les ordres déjà utilisés pour ce parcours (hors cette étape)
                    $ordres_utilises = [];
                    $stmt = $pdo->prepare("SELECT ordre_etape FROM etapes WHERE id_parcours = ? AND id_etape != ? ORDER BY ordre_etape");
                    $stmt->execute([$etape['id_parcours'], $id_etape]);
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        $ordres_utilises[] = (int)$row['ordre_etape'];
                    }
                    // Calculer le nombre total d'étapes pour le parcours
                    $stmt2 = $pdo->prepare("SELECT COUNT(*) FROM etapes WHERE id_parcours = ?");
                    $stmt2->execute([$etape['id_parcours']]);
                    $nb_etapes = (int)$stmt2->fetchColumn();
                    $max_ordre = max($nb_etapes, (int)$etape['ordre_etape']);
                    ?>
                    <select id="ordre_etape" name="ordre_etape" required>
                        <?php for ($i = 1; $i <= $max_ordre; $i++): ?>
                            <option value="<?= $i ?>" <?= ($i == $etape['ordre_etape']) ? 'selected' : (in_array($i, $ordres_utilises) ? 'disabled' : '') ?>><?= $i ?><?= in_array($i, $ordres_utilises) && $i != $etape['ordre_etape'] ? ' (occupé)' : '' ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
