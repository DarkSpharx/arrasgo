...
$debug_messages = [];
$debug_messages[] = '<div style="background: #ffeeba; color: #856404; padding: 12px; border-radius: 8px; margin: 16px 0;">DEBUG : Début du script add_chapitre.php</div>';
...
if ($id_etape <= 0) {
    $debug_messages[] = '<div style="background: #f8d7da; color: #721c24; padding: 12px; border-radius: 8px; margin: 16px 0;">Erreur : id_etape manquant ou invalide (' . htmlspecialchars($id_etape) . ')</div>';
} else {
    $stmt = $pdo->prepare('SELECT id_etape FROM etapes WHERE id_etape = ?');
    $stmt->execute([$id_etape]);
    if (!$stmt->fetch()) {
        $debug_messages[] = '<div style="background: #f8d7da; color: #721c24; padding: 12px; border-radius: 8px; margin: 16px 0;">Erreur : étape non trouvée en base pour id_etape = ' . htmlspecialchars($id_etape) . '</div>';
    }
}
$error = '';
...
<body>
    ...
    <?php
    // Affiche les messages de debug après le header, pour éviter les erreurs de session/header
            ...
            <div class="form-container">
                <form method="POST" enctype="multipart/form-data" action="add_chapitre.php">
                    ...
                    <div class="form-group-horizontal">
                        ...
                        <input type="text" id="titre_chapitre" name="titre_chapitre" required>
                    </div>
